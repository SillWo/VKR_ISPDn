...
import type { SecurityLevelDataCategoryKey, SecurityLevelFormValues } from "../../../entities/security-level/model/types";

type DataCategorySwitchesProps = {
  control: Control<SecurityLevelFormValues>;
  errors: FieldErrors<SecurityLevelFormValues>;
  disabled?: boolean;
  disabledCategories?: SecurityLevelDataCategoryKey[];
};

export function DataCategorySwitches({
  control,
  errors,
  disabled,
  disabledCategories = [],
}: DataCategorySwitchesProps) {
  const disabledCategorySet = new Set(disabledCategories);
      ...
      >
        {dataCategoryOptions.map((category) => (
          <Controller
            ...
            control={control}
            render={({ field }) => {
              const isCategoryDisabled = disabled || disabledCategorySet.has(category.value);
