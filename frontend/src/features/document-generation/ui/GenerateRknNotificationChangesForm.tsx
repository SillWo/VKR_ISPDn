...
import type {
  ...
  RknAccessPersonType,
  RknNotificationChangesFormValues,
...
const rknNotificationChangesSchema = z.object({
  changeDate: requiredText("Укажите дату появления события"),
  mainOfficeReg: requiredText("Укажите регистрационный номер организации в реестре РКН"),
...
function mapToPayload(values: RknNotificationChangesFormValues): GenerateGlobalDocumentPayload {
  return {
    documentType: "RKN_notification_changes",
    manualData: {
      change_date: values.changeDate,
      main_office_reg: values.mainOfficeReg.trim(),
...
type AccessPersonNameFieldProps = {
  control: ReturnType<typeof useForm<RknNotificationChangesFormValues>>["control"];
  index: number;
  disabled: boolean;
  error?: string;
};
...
export function GenerateRknNotificationChangesForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<RknNotificationChangesFormValues>({
    resolver: zodResolver(rknNotificationChangesSchema),
    defaultValues: {
      changeDate: "",
      mainOfficeReg: "",
      rknAccessPersons: [],
  ...
  const mutation = useMutation({
    mutationFn: (values: RknNotificationChangesFormValues) => generateGlobalDocument(mapToPayload(values)),
      ...
      <Stack spacing={3}>
        ...
        <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
          <TextField
            ...
            type="date"
            disabled={disabled}
            error={Boolean(errors.changeDate)}
          ...
          <TextField
            ...
            disabled={disabled}
            error={Boolean(errors.mainOfficeReg)}
            helperText={errors.mainOfficeReg?.message}
            fullWidth
            {...register("mainOfficeReg")}
          />
        </Stack>
