...
import type {
  ...
  PrikazOtvetZaPdnFormValues,
} from "../../../entities/document/model/types";
import { HttpError } from "../../../shared/api/httpClient";
import { requiredText } from "../../../shared/lib/validation";

const prikazOtvetZaPdnSchema = z.object({
  orderNumber: requiredText("Укажите номер приказа"),
});

function mapToPayload(values: PrikazOtvetZaPdnFormValues): GenerateGlobalDocumentPayload {
  return {
...
export function GeneratePrikazOtvetZaPdnForm() {
  const [downloadError, setDownloadError] = useState<string | null>(null);
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<PrikazOtvetZaPdnFormValues>({
    resolver: zodResolver(prikazOtvetZaPdnSchema),
    defaultValues: {
      orderNumber: "",
    },
  });
  ...
  const mutation = useMutation({
    mutationFn: (values: PrikazOtvetZaPdnFormValues) => generateGlobalDocument(mapToPayload(values)),
