...
class CryptoToolRepository:
    ...
    def list(self) -> list[CryptoTool]:
        statement = select(CryptoTool).order_by(CryptoTool.name.asc(), CryptoTool.id.asc())
        return list(self.db.scalars(statement).all())

    def list_options(self) -> list[CryptoTool]:
        return self.list()

    def get_by_id(self, crypto_tool_id: int) -> CryptoTool | None:
        statement = (
            select(CryptoTool)
            .options(selectinload(CryptoTool.ispdn_cards))
            .where(CryptoTool.id == crypto_tool_id)
        )
        return self.db.scalars(statement).first()

    def get_many_by_ids(self, crypto_tool_ids: list[int]) -> list[CryptoTool]:
        if not crypto_tool_ids:
            return []
        statement = (
            select(CryptoTool)
            .where(CryptoTool.id.in_(crypto_tool_ids))
            .order_by(CryptoTool.name.asc(), CryptoTool.id.asc())
        )
        return list(self.db.scalars(statement).all())

    def create(self, payload: CryptoToolCreate) -> CryptoTool:
        crypto_tool = CryptoTool(**payload.model_dump())
    ...
    def is_linked_to_ispdn(self, crypto_tool_id: int) -> bool:
        crypto_tool = self.get_by_id(crypto_tool_id)
    ...
    def count_existing_ids(self, crypto_tool_ids: list[int]) -> int:
        if not crypto_tool_ids:
            return 0
        statement = select(func.count(CryptoTool.id)).where(CryptoTool.id.in_(crypto_tool_ids))
        return int(self.db.scalar(statement) or 0)

    def get_ispdn_cryptography(self, ispdn_id: int) -> IspdnCryptographySettings | None:
        statement = (
    ...
    def set_ispdn_cryptography(
        ...
        ispdn.crypto_tools = self.get_many_by_ids(payload.crypto_tool_ids) if payload.uses_cryptography else []
        self.db.commit()
        return self.get_ispdn_cryptography(ispdn.id) or settings
