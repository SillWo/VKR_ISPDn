...
class ControlEventRepository:
    ...
    def list_options(self) -> list[ControlEvent]:
        statement = select(ControlEvent).order_by(ControlEvent.name.asc())
        return list(self.db.scalars(statement).all())

    def get_by_id(self, control_event_id: int) -> ControlEvent | None:
        statement = (
            select(ControlEvent)
            .options(selectinload(ControlEvent.files))
            .where(ControlEvent.id == control_event_id)
        )
        return self.db.scalars(statement).first()

    def get_by_name(self, name: str) -> ControlEvent | None:
        statement = select(ControlEvent).where(ControlEvent.name == name)
        return self.db.scalars(statement).first()

    def create(self, payload: ControlEventCreate) -> ControlEvent:
        control_event = ControlEvent(**payload.model_dump())
        self.db.add(control_event)
        self.db.commit()
        self.db.refresh(control_event)
        return self.get_by_id(control_event.id) or control_event
