...
class TaskEventService:
    ...
    def list_events(
        ...
        if responsible_employee_id is not None:
            self._ensure_employee_exists(responsible_employee_id)
    ...
    def get_event(self, task_event_id: int) -> TaskEvent:
        task_event = self.repository.get_event_by_id(task_event_id)
    ...
    def create_task(self, task_event_id: int, payload: TaskCreate) -> Task:
        task_event = self.get_event(task_event_id)
        self._ensure_task_payload_is_valid(payload)
        return self.repository.create_task(task_event, payload)

    def update_task(self, task_event_id: int, task_id: int, payload: TaskUpdate) -> Task:
    ...
    def list_actual_tasks_for_ispdn(self, ispdn_id: int) -> list[ActualTaskRead]:
        self._ensure_ispdn_exists(ispdn_id)
    ...
    def _ensure_task_payload_is_valid(self, payload: TaskCreate | TaskUpdate) -> None:
        ...
        if payload.responsible_employee_id is not None:
            self._ensure_employee_exists(payload.responsible_employee_id)
